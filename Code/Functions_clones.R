...
calc_tree_clones <- function(clones, total_clones) { 
  ...
  clone_tree[,1] <<- names(table(clones[,"Clone_ID"]))      # Clone_ID
  
  
  for ( i in 1:total_clones ) {
    st <- min( which(clones[,"Clone_ID"] == clone_tree[,"Clone_ID"][i]))
    en <- max( which(clones[,"Clone_ID"] == clone_tree[,"Clone_ID"][i]))
    clone_tree[i,"Time_start"] <<- clones[st,"Birthday"]
    clone_tree[i,"Time_end"]   <<- clones[en,"Time"]   
    clone_tree[i,"Parent_ID"]  <<- clones[st,"Parent_ID"] 
