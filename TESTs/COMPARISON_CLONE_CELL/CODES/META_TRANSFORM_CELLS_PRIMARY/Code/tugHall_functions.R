...
hallmark <- setRefClass(
    ...
    methods = list(
        ...
        read = function(file, names) {
            ...
            Ha_sum = sum(Ha0_w) ### ifelse( sum(Ha0_w) == 0, 1, sum(Ha0_w) ) # sum(Ha0_w)
            Hi_sum = sum(Hi0_w) ### ifelse( sum(Hi0_w) == 0, 1, sum(Hi0_w) ) # sum(Hi0_w)
            Hd_sum = sum(Hd0_w) ### ifelse( sum(Hd0_w) == 0, 1, sum(Hd0_w) ) # sum(Hd0_w)
            Hb_sum = sum(Hb0_w) ### ifelse( sum(Hb0_w) == 0, 1, sum(Hb0_w) ) # sum(Hb0_w)
            Him_sum = sum(Him0_w) ### ifelse( sum(Him0_w) == 0, 1, sum(Him0_w) ) # sum(Him0_w)
            Ha_w <<- Ha0_w/Ha_sum
            Hi_w <<- Hi0_w/Hi_sum
            Hd_w <<- Hd0_w/Hd_sum
            Hb_w <<- Hb0_w/Hb_sum
        ...
        updateCell = function(cell1, F) {
          # Apoptosis
            ...
            ### TEST: to d_0 = 1  
            cell1$d = 1.0   ### cell1$Hd + 0.25     # d_0 = 0.25
            ...
            ### TEST: for invasion transformation, set im = 0.5
            cell1$im = 0.5
...
# write log file
write_log <- function(genefile, cellfile, geneoutfile, celloutfile, logoutfile, i, E0, F0, m, uo, us, s, k, censore_n, censore_t) {
    data <- c("#_simulation", "genefile", "cellfile", "geneoutfile", "celloutfile", "logoutfile",
              "E", "F", "m", "uo", "us", "s", "k", "censore_n", "censore_t")
    data <- rbind(data, c(i, genefile, cellfile, geneoutfile, celloutfile, logoutfile,
...
#              env$posdriver, env$pospasngr, '-', '-', '-')
    ...
    if (length(cells) > 0 & isFirst & FALSE) {
...
model <- function(onco, hall, genefile, cellfile, geneoutfile, celloutfile, logoutfile, E0, F0, m, uo, us, s, k, censore_n, censore_t) {
