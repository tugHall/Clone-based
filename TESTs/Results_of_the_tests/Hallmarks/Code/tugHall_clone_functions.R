...
clone <- setRefClass(
    ...
    methods = list(
        # Initialize
        initialize = function(gene_size, id=1, parent=0, c=0, d=0, i=1, m=m0,  N_cells = 1, 
...
trial <- function(clone1) { 
    ...
    N_die = 0    # The number of cells to die 
    
    # trial for Environmental death of cell 
    N_die = N_die + rbinom(1, clone1$N_cells, clone1$k)  # The number of cells to die due to the Environmental death of cells in clone
    
    # Apoptosis trial
    N_die = N_die + rbinom(1, clone1$N_cells, clone1$a)

    # invasion / metastasis trial 
    if (clone1$im > 0) {
      if (!clone1$invasion) {
        N_die = N_die + rbinom(1, clone1$N_cells, (1 - clone1$im))
        clone1$invasion = ifelse(clone1$im == 1,TRUE,FALSE)
        }
    }
    
    # The new number of cells in the clone: 
    # STOP the killing cells for TEST !!!!!!!!!!!!!!!!!
    ...
    if (clone1$c > 50) {
      N_new = rbinom(1, N_new, (1 - clone1$i))
    }
    
    
    # Devide trial
        
    N_new = rbinom(1, N_new, clone1$d)   # The ! FINAL ! number of cells to split, this number is the output value of function "trial"
    ...
    # STOP division of cells for TEST !!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    # if (p < 1) N_new_clones = rbinom(1, 2 * N_new, p ) else N_new_clones = 2 * N_new
    
    N_new_clones = rbinom(1, 2 * N_new, 1 - p0 )   # coefficient 2, because of parent and children independent mutations 
...
write_cloneout <- function(outfile, env, clones, isFirst) {
...
model <- function(genefile, clonefile, geneoutfile, cloneoutfile, logoutfile, 
                  E0, F0, m0, uo, us, s0, k0, censore_n, censore_t) {
    write_log(genefile, clonefile, geneoutfile, cloneoutfile, logoutfile, 
              E0, F0, m0, uo, us, s0, k0, censore_n, censore_t)   # write input parameters
    ...
    isFirst = TRUE
    write_cloneout(cloneoutfile, env, clones, isFirst)     #  write initial clones
    ...
    while(length(clones) > 0 && censore_n > cells_number && env$T < censore_t) {
       ...
       N_clones_new = unlist(lapply(clones, trial)) 
        ...
        if ( N_new > 0) {
          ...
          LargeNumber <- as.integer( 1 + 3 * N_new / (1 - p0) )
          num_mut <- numeric(0)
          A <- numeric(0)
          
          while (length(num_mut) < N_new ) {
            
            A <- rbinom(LargeNumber, sumCDS, m0) 
            num_mut <- c( num_mut, A[A>0] )
          }
          num_mut <- num_mut[1:N_new]
        ...
        write_cloneout(cloneoutfile, env, clones, isFirst)
...
### This is input parameters and simulation, which should be found in "../tugHall_2-clone.R"   script 
if (FALSE) { 
         
         library(stringr)
          
         genefile <- 'gene_cds2.txt'     #args[1]                # gene file - "gene_cds2.txt"
         clonefile <- 'cloneinit.txt'      #args[2]                # initial Cells 
         geneoutfile <- 'geneout.txt'    #args[3]             # Gene Out file with Hallmarks - "gene out"
         cloneoutfile <- 'cloneout.txt'    #args[4]             # output information of simulation 
         logoutfile <-  'log.txt'        #args[5]              # log file to save the input information of simulation - "log.txt"
         E0 <- 2E-30                     #as.numeric(args[6])          # (1E-1, 1E-2, 1E-3, 1E-4, 1E-5)
         F0 <- 1E30                      #as.numeric(args[7])          # (10^1, 10^2, 10^3, 10^6, 10^9, 10^12)
         m0 <-  1E-8                       #as.numeric(args[8])           # mutation probability (1*10e-11, 5*10e-11, 1*10e-10, 5*10e-10, 1*10e-9, 5*10e-9, 1*10e-8, 5*10e-8)
         uo <- 0.2                       #as.numeric(args[9])          # oncogene mutation probability
         us <- 0.1                       #as.numeric(args[10])         # suppressor mutation probability
         s0 <-  10                        #as.numeric(args[11])          # (10, 100)
         k0 <-  0.2                       #as.numeric(args[12])          # Environmental death probability
         censore_n <- 30000               #as.numeric(args[14])  # Max cell number where the program forcibly stops
         censore_t <- 100                #as.numeric(args[15])  # Max time where the program forcibly stops
